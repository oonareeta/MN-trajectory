...
for (i in disease) {
  ...
  df_disease = df %>%
    dplyr::rename(#risk_score = predicted_label,
  ...
  if (j == 5) {
    df_disease = df_disease %>%
      mutate(
        time_to_dg_y = time_to_dg,
        event_y = disease)
  } else {
    df_disease = df_disease %>%
      mutate(
        time_to_dg_y = ifelse(disease == 1 & time_to_dg >= j*365.24, 2000, time_to_dg),
        event_y = ifelse(disease == 1 & time_to_dg_y <= j*365.24, 1, 0))
  }
  
  # Calculate the AUC
  auc = pROC::roc(df_disease$event_y, df_disease$risk_score, plot=TRUE);auc
  
  # Confusion matrix
  conf_matrix <- confusionMatrix(as.factor(df_disease$predicted_label), as.factor(df_disease$event_y), positive = "1")
  ...
  ggsave(plot = g,
         filename = paste0(results, "/", gsub("de_novo_AML", "de novo AML", i), "/Prediction_at_", j, "_year.png"),
...
# Export
writexl::write_xlsx(metrics1, paste0(results, "/metrics_prediction_1_3_5_years.xlsx"))
